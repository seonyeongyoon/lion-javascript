...
//함수가 실행되는 것이 아니라 본문 자체가 넘어간다(내보내진다).
function first(){
  let x = 10;
...
//currying function
//first()();//15. 뒤의 함수는 second를 의미
const value = first() //15 실행 안되었던 함수(second)를 () 로 실행시켜준다
//value() = first()()

function counter(){
  let count = 0;

  function incre(){//카운터라는 함수가 인크리를 퉤하고 내뱉음
    count++;
    return count;
  }
...
let result = counter();//리절트가 인크리를 받음?
let result2 = counter();//리절트가 인크리를 받음?
let result3 = counter();//리절트가 인크리를 받음?
let result4 = counter();//리절트가 인크리를 받음?
//각 각 환경이 따로 저장됨
